...
func main() {
	...
	// রাউট সেটআপ
	http.HandleFunc("/books", handleBooks)         // GET & POST
	http.HandleFunc("/edit-book", editBook)       // PUT
	http.HandleFunc("/update-status", updateStatus) // PATCH
	http.HandleFunc("/delete-book", deleteBook)     // DELETE
...
func handleBooks(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		_, err := db.Exec("INSERT INTO books (title, author, status) VALUES ($1, $2, 'Available')", b.Title, b.Author)
		if err != nil {
			http.Error(w, err.Error(), 500)
			return
		}
...
func editBook(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPut {
		...
		_, err := db.Exec("UPDATE books SET title=$1, author=$2 WHERE id=$3", b.Title, b.Author, b.ID)
		if err != nil {
			http.Error(w, err.Error(), 500)
			return
		}
